...
int main(void) {
    ...
    if (Uart_init_specific(UART_DEVICE_ID_EPS) != ERROR_NONE) {
    ...
    while (test_complete == false) {
        switch(test_step) {
            ...
            case 0:
                DEBUG_INF("----- STEP 0 -----");
                if (Uart_send_bytes(UART_DEVICE_ID_EPS, send_data, data_size) != ERROR_NONE) {
            ...
            case 2:
                DEBUG_INF("----- STEP 2 -----");
                if (Uart_recv_bytes(UART_DEVICE_ID_EPS, recv_data, data_size) != ERROR_NONE) {
                    Debug_exit(1);
                }
                if (Uart_step() != ERROR_NONE) {
                    Debug_exit(1);
