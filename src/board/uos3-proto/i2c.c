...
static void InitI2C(uint32_t i2cbase, uint32_t sysctl_i2c,uint32_t sysctl_gpio,uint32_t gpio_port,
            uint32_t pin_scl,uint32_t pin_sda,uint8_t mux_scl, uint8_t mux_sda) // initialise board for I2C capability
{
...
uint8_t *I2CReceiveGP(uint8_t slave_addr, uint8_t reg,uint8_t strlen, uint32_t i2cbase, bool reverse) // returns zero pointer if an error...
 ...
 // before can receive data, have to tell slave what register to start at
    ...
    if (strlen==1) { 
                     ...
                     while(I2CMasterBusy(i2cbase));
                     I2CReceiveBuffer[0]=(uint8_t) I2CMasterDataGet(i2cbase); // exit with ptr to zero terminated two byte string
        ...
        I2CMasterControl(i2cbase, I2C_MASTER_CMD_BURST_RECEIVE_START);
         while(I2CMasterBusy(i2cbase));
        I2CReceiveBuffer[0]=(uint8_t) I2CMasterDataGet(i2cbase);
        uint8_t i=1;
        while (i<strlen-1) // do the middle bytes
         { 
          ...
          I2CReceiveBuffer[i]=(uint8_t) I2CMasterDataGet(i2cbase);        
           i++;
         }

         I2CMasterControl(i2cbase, I2C_MASTER_CMD_BURST_RECEIVE_FINISH);
         while(I2CMasterBusy(i2cbase));
         I2CReceiveBuffer[strlen-1]=(uint8_t) I2CMasterDataGet(i2cbase);
...
uint8_t I2CReceive(uint8_t slave_addr, uint8_t reg,uint32_t i2cbase)
{
  uint8_t *ans=I2CReceiveGP(slave_addr,reg,1,i2cbase,false);
...
uint16_t I2CReceive16(uint8_t slave_addr,uint8_t reg,uint32_t i2cbase) // lets get 2 byte value 
 ...
 uint8_t *ans=I2CReceiveGP(slave_addr,reg,2,i2cbase,false);
  if (ans==NULL) return 0; // error handling
...
uint16_t I2CReceive16r(uint8_t slave_addr,uint8_t reg,uint32_t i2cbase) // lets get 2 byte value reversed (for other endian words)
 {
 uint8_t *ans=I2CReceiveGP(slave_addr,reg,2,i2cbase,true);
  if (ans==NULL) return 0; // error handling
 return *(uint16_t *)ans;
 }
